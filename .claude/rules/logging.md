...
> Scope: Java/Spring modules (`server`, `persistence`) only. `entrance-engine`/`entrance-dsl`/`entrance-plans` are pure functions with no logging; `entrance-batch` may log its run but follows its own conventions in [`entrance/CLAUDE.md`](../../entrance/CLAUDE.md).
