...
class Plan2026Test {
    ...
    @Test
    fun `총 모집정원은 72명이다`() {
        assertEquals(72, plan2026.totalCapacity)
        assertEquals(36, plan2026.major("SW").capacity)
        assertEquals(18, plan2026.major("IOT").capacity)
        assertEquals(18, plan2026.major("AI").capacity)
    }

    @Test
    fun `일반전형 64명, 특별전형 8명이다`() {
        assertEquals(64, plan2026.resolvedQuota("GEN"))
        assertEquals(8, plan2026.resolvedQuota("SPE"))
    }

    @Test
    fun `특별전형 미충원분과 탈락자는 일반전형으로 이동한다`() {
        val spe = plan2026.screening("SPE")
    ...
    fun `사회통합전형 세부 유형은 5종이다`() {
        assertEquals(5, plan2026.screening("SPE").subTypes.size)
    }

    @Test
    fun `정원 외 전형 - 국가보훈 2명(3% 이내), 특례 1명(2% 이내)`() {
        val veterans = plan2026.screening("EXT_VETERANS")
        val special = plan2026.screening("EXT_SPECIAL")
    ...
    fun `정원 외 전형은 1차 합격선 이내일 때만 정원 외로 전형하고, 초과 시 사회통합전형에 편입된다`() {
        listOf("EXT_VETERANS", "EXT_SPECIAL").forEach { code ->
            val screening = plan2026.screening(code)
    ...
    fun `졸업예정자 일반교과 학기 배점은 18-45-45-72다`() {
        val scheme = plan2026.grading.schemes.getValue(CANDIDATE) as TranscriptGrading
    ...
    fun `졸업자 일반교과 학기 배점은 36-36-54-54다`() {
        val scheme = plan2026.grading.schemes.getValue(GRADUATE) as TranscriptGrading
        val points = scheme.generalSubjects.semesterPoints.mapValues { it.value.toInt() }
    ...
    fun `1차 전형은 1_3배수 총원제 선발이다`() {
        val first = plan2026.round("FIRST")
    ...
    fun `1차 동점자 기준 - 일반교과, 학기 순(3-1, 2-2, 2-1, 1-2), 비교과`() {
        val tiebreakers = plan2026.round("FIRST").tiebreakers
    ...
    fun `최종 동점자 기준은 역량검사, 심층면접 순으로 시작한다`() {
        val tiebreakers = plan2026.round("SECOND").tiebreakers
    ...
    fun `학과 배정 - 3지망 필수, 정원 외는 학과당 최대 2명`() {
        assertEquals(3, plan2026.majorAssignment.choiceCount)
        assertEquals(2, plan2026.majorAssignment.extraScreeningCapPerMajor)
    }

    @Test
    fun `예비합격은 일반전형에서 정원의 3% 범위다`() {
        val waitlist = plan2026.waitlist!!
    ...
    fun `추가모집은 일반전형만, 1차 환산점수 기준이다`() {
        val additional = plan2026.additionalRecruitment!!
